 ...
 * 非线程安全
 */
public class AliyunSpeechRecognizer extends SpeechReqProtocol implements Closeable {
    static Logger logger = LoggerFactory.getLogger(AliyunSpeechRecognizer.class);
    ...
    public void setEnableITN(Boolean enableITN) {
        if (null == enableITN) {
            payload.remove(Constant.PROP_ASR_ENABLE_ITN);
        } else {
     ...
     * @return
     */
    public boolean isEnableITN() {
        return (Boolean) payload.getOrDefault(Constant.PROP_ASR_ENABLE_ITN, false);
    }

    /**
     * 设置开启语音检测
     *
     * @param enableVoiceDetection
     */
    public void setEnableVoiceDetection(Boolean enableVoiceDetection) {
        if (null == enableVoiceDetection) {
            payload.remove("enable_voice_detection");
        } else {
            payload.put("enable_voice_detection", enableVoiceDetection);
        }
    }
    ...
    public boolean isEnableVoiceDetection() {
        return (Boolean) payload.getOrDefault("enable_voice_detection", false);
    }

    /**
     * 设置静音开始时长（毫秒）
     *
     * @param maxStartSilence
     */
    public void setMaxStartSilence(Long maxStartSilence) {
        if (null == maxStartSilence) {
            payload.remove("max_start_silence");
        } else {
            payload.put("max_start_silence", maxStartSilence);
        }

    }

    public Long getMaxStartSilence() {
        return (Long) payload.get("max_start_silence");
    }
     ...
     * @param maxEndSilence
     */
    public void setMaxEndSilence(Long maxEndSilence) {
        if (null == maxEndSilence) {
            payload.remove("max_end_silence");
        } else {
            payload.put("max_end_silence", maxEndSilence);
        }
    }

    public Long getMaxEndSilence() {
        return (Long) payload.get("max_end_silence");
    }

    public AliyunSpeechRecognizer(NlsClient client, AliyunSpeechRecognizerListener listener) throws Exception {
        Connection conn = client.connect(listener);
        this.conn = conn;
        afterConnection(listener);
    }

    public AliyunSpeechRecognizer(NlsClient client, String token, AliyunSpeechRecognizerListener listener) throws Exception {
    ...
    protected void afterConnection(AliyunSpeechRecognizerListener listener) {
